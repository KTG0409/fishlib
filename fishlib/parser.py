...
# ONLY these aliases can match without the category name in the text.
# If an alias is NOT here, it requires the category name to be present.
# This prevents "ABALONE CHILEAN" from matching as Chilean Sea Bass, etc.
...
_P3_WHITELIST = {
    ...
    'FLUKE',                                            # Flounder
    'PICKEREL',                                         # Walleye-adjacent (walleye itself must appear with 'PIKE' or in walleye category)
    ...
    'FLOUNDER',                                         # Flounder
    'TILAPIA',                                          # Tilapia
    'ROCKFISH', 'BOCACCIO', 'SEBASTES',                 # Rockfish
    'SWAI', 'PANGASIUS', 'BASA',                        # Swai
    'SOCKEYE', 'CHINOOK', 'KOKANEE',                    # Salmon subspecies
    'SCROD',                                            # Atlantic cod
    'KUMAMOTO',                                         # Pacific oyster
    'AHI', 'MAGURO', 'ALBACORE', 'SKIPJACK', 'BONITO', # Tuna
    ...
    'RED SNAPPER', 'YELLOWTAIL SNAPPER', 'VERMILION SNAPPER',
    'LANE SNAPPER', 'MANGROVE SNAPPER', 'SILK SNAPPER',
    'SEA SCALLOP', 'BAY SCALLOP', 'DIVER SCALLOP', 'DAY BOAT',
    'DOVER SOLE', 'PETRALE SOLE', 'LEMON SOLE', 'REX SOLE', 'GRAY SOLE',
    'GREEN LIP', 'BLUE MUSSEL',
    'OCEAN TROUT', 'RAINBOW TROUT', 'STEELHEAD TROUT', 'BROOK TROUT',
    'NORTHERN PIKE',
    'EUROPEAN SEA BASS', 'MEDITERRANEAN SEA BASS', 'ASIAN SEA BASS',
    'RED KING', 'GOLDEN KING', 'BLUE KING',
    'BLACK TIGER', 'GIANT TIGER',
    'ROYAL RED', 'GULF PINK', 'GULF BROWN',
    'SUMMER FLOUNDER', 'WINTER FLOUNDER',
    'PACIFIC ROCKFISH', 'ROCK COD', 'SNAPPER ROCKFISH',
    'RED ROCKFISH', 'VERMILION ROCKFISH', 'CANARY ROCKFISH',
...
def _is_p3_safe(alias: str) -> bool:
    """
    Check if an alias is safe for Priority 3 (no category name in text).
    Only whitelisted aliases and scientific names are allowed.
    """
    if alias in _P3_WHITELIST:
        return True
    # Scientific names (contain lowercase letters) are always safe
    if any(c.islower() for c in alias):
        return True
    return False
...
def _extract_species(text: str) -> Optional[Dict[str, Any]]:
    ...
    # --- Priority 3: SAFE alias only, no category name in text ---
    # Only allows aliases that are specific enough to uniquely identify a species
    # without the category name present. Generic words like CHILEAN, BLUE, RED,
    # DOMESTIC, ATLANTIC, etc. are NOT allowed here — they cause false matches
    # (e.g., "ABALONE CHILEAN" matching as Chilean Sea Bass).
    # 
    # An alias is "P3-safe" if it is:
    #   - A multi-word species name (e.g., "CHILEAN SEA BASS", "WALLEYE PIKE")
    #   - A unique species name not shared with common adjectives (e.g., "SABLEFISH")
    #   - A scientific name (contains lowercase or 2+ words)
    #   - Listed in the explicit P3_SAFE_ALIASES set below
